...
// Windows stores every toast's full XML — including the `launch`/`activationType` the shell replays
// when you click the real banner — in wpndatabase.db, keyed by the SAME Id the UserNotificationListener
// hands us (verified: DB Id == UserNotification.Id). The listener API hides those args, so to make a
// banner click open the exact message/photo (not just the app), we read them straight out of the DB.
// ponytail: use the system winsqlite3.dll (no NuGet). The row is checkpointed into the main .db by the
// time a banner is clicked, so a plain read-only snapshot of just the .db is enough (no -wal juggling).
    ...
    public static (string launch, string activationType) LaunchFor(uint id)
    {
        ...
        try
        {
            ...
            using (var d = new FileStream(tmp, FileMode.Create, FileAccess.Write))
                s.CopyTo(d); // the live db is locked → snapshot it
