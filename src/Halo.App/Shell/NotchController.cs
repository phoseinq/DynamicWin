...
internal sealed class NotchController
{
    ...
    private void Frame()
    {
        ...
        else if (visibility.Action == NotchVisibilityAction.ShowAndRender)
        {
            ...
            _notch.SetVisible(!_hiddenForEmpty);
            _lastFg = IntPtr.Zero;
            Apply(_progress);
        }
        ...
        bool open = (hovered || notice) && !_empty && _notif == null && !_moving;
        ...
        bool emptyHidden = _shrink >= 1f && _notif == null && _progress <= 0.02f && !Privacy.Active;
        if (emptyHidden != _hiddenForEmpty)
        {
            _hiddenForEmpty = emptyHidden;
            if (!_hiddenForFullscreen) _notch.SetVisible(!emptyHidden);
        }
