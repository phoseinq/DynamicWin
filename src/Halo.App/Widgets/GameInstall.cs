...
// Gaming Services (GDK games — Roblox & co.) bypass AppInstallManager AND Delivery Optimization
// entirely (both verified empty during a live Roblox download). They stage into
// <drive>:\XboxGames\<GUID>\Content, so progress is read from disk: folder growth = downloaded bytes.
// The staged MicrosoftGame.config gives the name + StoreId; the Store display-catalog gives the total
// download size for that StoreId → a real filling %, not just a spinner. If the catalog can't be
// reached the pill falls back to live MB + spinner.
//
// Activity is detected by the folder SIZE changing between scans (no FileSystemWatcher — a recursive
// watch over an actively-downloading game floods thousands of events/sec and starved the render). Idle
// → one size scan every 30s; active → every 8s. A folder already on disk at startup is baselined
// silently so a finished install never looks like a running download.
internal static class GameInstall
{
    ...
    // the game's own logo (staged in Content/Assets) so the pill shows Roblox's icon, not the Store bag
    ...
    public static bool Poll(out string name, out long done, out long total, out bool stalled)
    {
        ...
        string? folder = FindStagingFolder();      // cheap: enumerate top-level XboxGames dirs
        ...
        if (folder != Volatile.Read(ref _folder))
        {
            ...
            if (_startupDone) Interlocked.Exchange(ref _lastGrewTick, Environment.TickCount64); // appeared while running = new download
        }
        _startupDone = true;
        FetchTotalOnce();
        ...
        long interval = active ? 20000 : 40000;    // 20s while live: a recursive size scan on a big,
                                                   // actively-downloading tree thrashes the disk against
                                                   // the download itself → scan sparingly (coarser MB is fine)
        ...
        if (sinceGrew > 90_000) return false;      // long idle / finished → stop owning the pill
        lock (_lock) { name = _name ?? "Xbox game"; done = _bytes; total = _total; }
        stalled = !active;                         // between growth bursts → static, not spinning
        return true;
    }
    ...
    private static void Rescan()
    {
        ...
        lock (_lock)
        {
            ...
            // grew >256KB since last scan → live. First scan just baselines (a finished install measured
            // at startup must NOT count as growth).
    ...
    // one catalog request per game for the total download size (MaxDownloadSizeInBytes across packages)
    ...
    // name + StoreId + logo path from MicrosoftGame.config (falls back to the appxmanifest DisplayName)
    private static (string? name, string? storeId, string? logo) ReadConfig(string dir)
    {
        ...
        try
        {
            ...
            if (File.Exists(cfg))
            {
                ...
                // prefer the 150x150 tile, fall back to the store logo — resolve relative to Content/
