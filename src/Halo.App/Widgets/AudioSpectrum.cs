...
// Real STEREO spectrum for the equalizer: WASAPI loopback capture of the default render device →
// per-channel FFT → 9 bars laid out as a stereo wing: [L treble … L bass | BASS | R bass … R treble].
// Left half follows the LEFT speaker, right half the RIGHT; bass sits at the centre so a shout or
// kick lifts the middle. Zero deps, hand-rolled COM. The capture thread runs only while the
// equalizer polls Bands() (5s grace). Fallback: Available=false → widget keeps the old peak anim.
internal static class AudioSpectrum
{
    public const int BandCount = 9;
    private const int Ch = 5;                      // bands per channel (bass..treble); centre shares bass
    private static readonly float[] _bands = new float[BandCount];
    public static volatile bool Available;

    private static Thread? _thread;
    private static long _until; // last poll + grace — thread parks when nobody wants bars

    // called per equalizer frame; returns smoothed 0..1 per bar (a copy — caller-safe)
    ...
    private const int N = 1024;                    // FFT size (~21ms @48k — snappy)
    ...
    private static void Loop()
    {
        while (true)
        {
            try
            {
                ...
                Capture(); // runs until idle grace expires or the device dies; then re-acquire
            }
            catch { Available = false; }
            Thread.Sleep(500); // device switch / init failure → retry
        }
    }
    ...
    private static void Capture()
    {
        ...
        try
        {
            ...
            if (bits != 32 || channels < 1 || rate < 8000) return; // mix format is float32 in practice

            const uint LOOPBACK = 0x00020000;
            if (ac.Initialize(0, LOOPBACK, 2_000_000, 0, fmtPtr, IntPtr.Zero) != 0) return; // 200ms buffer
            ...
            while (Environment.TickCount64 <= _until)
            {
                ...
                if (now >= nextFft)
                {
                    nextFft = now + 25; // ~40fps analysis — keeps perceived latency low
    ...
    private static void ComputeBands(float[] win, int rate)
    {
        ...
        // side bars: 4 log bands per channel, treble at the edges, bass beside the centre
        ...
        for (int b = 0; b < nb; b++)
        {
            ...
            float tilt = b * 3.2f;
            target[nb - 1 - b] = BandValue(_reL, _imL, rate, lo, hi, tilt);              // L: treble→bass
            target[BandCount - nb + b] = BandValue(_reR, _imR, rate, lo, hi, tilt);      // R mirrored
        }

        // CENTRE BAR = the announcer: true centre-channel extraction (mid − side power) over the
        // voice range. Centre-panned speech → mid strong/side ~0 → tall; wide stereo music → side
        // cancels it → the centre stays theirs. FFT is linear, so mid/side come free from L/R bins.
        {
            ...
            target[BandCount / 2] = Math.Clamp((db + 62f) / 35f, 0f, 1f); // hotter mapping: it's the star
        }

        // RELATIVE view over the SIDE bars only ("۰تا۱۰۰ با ۰تا۱۰۰ نباید برابر باشه"): the loud tone
        // towers over the rest (^1.6 contrast), overall level rides the dominant's loudness. The
        // CENTRE bar is exempt — it tracks the announcer directly and never gets crushed by music.
        float max = 0f;
        ...
        {
            float v = target[b];
            // fast rise, slower fall — lively but no flicker
    ...
    {
        int start = _ringPos; // newest N samples, windowed
        for (int i = 0; i < N; i++)
        {
    ...
    private static float BandValue(float[] re, float[] im, int rate, double lo, double hi, float tiltDb)
    {
        ...
        double rms = Math.Sqrt(sum / Math.Max(1, i1 - i0)) / N;
        float db = 20f * MathF.Log10((float)rms + 1e-9f);
        return Math.Clamp((db + 55f + tiltDb) / 40f, 0f, 1f); // floor −55dB
    }
