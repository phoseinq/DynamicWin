...
// DynamicWin's signature feature: a shelf you drag files onto to keep them handy. The notch registers a
// real OLE drop target (Interop/FileDropTarget) — a file dragged over the pill reveals the tray and it
// becomes a drop zone; dropped paths persist across runs. Inside the panel you can open a file (click),
// select several (Ctrl+click) to remove or drag out together, reorder by dragging up/down, or drag a
// file back OUT to Explorer / another app (Interop/FileDrag).
internal sealed class FileTray : IWidget
{
    ...
    private static readonly Color Accent = Color.FromArgb(120, 185, 255); // soft blue shelf accent
    ...
    // smooth reorder: each card's live top-left eases toward its grid slot instead of snapping
    ...
    // live reorder preview (indices into the snapshot); set by the controller during an in-panel drag
    ...
    public static void Add(string path)
    {
        ...
        lock (_lock)
        {
            ...
            _paths.Insert(0, full); // most-recent first
    ...
    // after a successful drag-out (dropped / pasted / sent) the file leaves the shelf automatically
    ...
    // the set a drag-out acts on: the whole selection if the grabbed row is part of it, else just that row
    ...
    // ── reorder (drag a row up/down inside the panel) ──
            ...
            lock (_lock)
                foreach (var line in File.ReadAllLines(StorePath))
                {
                    ...
                    if (p.Length > 0 && (File.Exists(p) || Directory.Exists(p))
                        ...
                        _paths.Add(p); // keep the saved order (already most-recent first)
                }
        }
        catch { }
    }
    ...
    // ── collapsed pill: one icon (or a small stack of file icons when several), then a label ──
    public void DrawCollapsed(Graphics g, int w, int h, float fade)
    {
        ...
        // several files: their icons lined up one behind another (leftmost = most recent, on top)
        int n = Math.Min(4, items.Length);
        float step = sz * 0.58f;
        for (int i = n - 1; i >= 0; i--)
        {
            // thin dark backing so overlapping tiles read as separate
    ...
    // ── expanded panel: a card grid (uses the width, shows more files than a single column) ──
    ...
    {
        if (fade <= 0.01f) { _settled = true; _anim.Clear(); return; } // collapsed: no pending glide
        var items = Snapshot();
        g.SmoothingMode = SmoothingMode.AntiAlias;
        ...
        // ease each card from where it is toward its target slot, so a reorder/insert glides, not snaps
        bool settled = true;
        ...
        for (int i = 0; i < shown; i++)
        {
            ...
            if (!_anim.TryGetValue(path, out var cur)) cur = target.Location; // a new card starts at its slot
            ...
            if (path == grabbed) { grabbedRect = rect; haveGrab = true; continue; } // lifted card drawn last (on top)
        ...
        _settled = settled;
        if (!settled) Interlocked.Increment(ref _version); // drive the next frame until the cards land
    ...
    // target cell index the cursor is over (for reorder), clamped to the visible cells
    ...
    // one file card: icon + name + folder, with an × on hover. Accent-tinted when selected / lifted.
    ...
    // buttons fire on mouse-down: the × per card, and the header "Remove N" when a selection exists.
    // Card body (open / drag / reorder) and Ctrl+click select are driven by NotchController.
    public IReadOnlyList<(RectangleF rect, Action<PointF> onClick)> Buttons(int w, int h)
    {
        ...
        {
            string path = items[i];
            list.Add((CellXRect(CellRect(i, w, h)), _ => Remove(path))); // × removes
        }
        return list;
    }

    // path of the file card under a panel-local point (null over the × or empty space)
    ...
    // rounded icon tile (real shell icon, or a tray glyph fallback)
    private static void DrawTile(Graphics g, float x, float y, float sz, float fade, Bitmap? icon)
    {
        ...
        else
        {
            ...
            g.FillPath(gb, path);
            DrawGlyph(g, box, ((char)0xE7B8).ToString(), sz * 0.5f, fade); // tray/inbox
        }
    }
