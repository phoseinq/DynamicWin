...
// Any AI tool beyond Claude/Codex: drop ~/.halo/agents/agent-<x>.json (schema in
// docs/generic-agents.md) and it gets a pill face, a strip circle with ring + number badge,
// and multi-session grouping by "name" — reusing StatusStore's slots/liveness/dedupe as-is.
internal sealed class GenericAgentWidget : IWidget
{
    ...
    // icon loaded from the file's "icon" path (cached per path) + session-number badge
