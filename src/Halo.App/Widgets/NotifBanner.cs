...
{
    public const int W = 470, SummaryH = 106;
    ...
    public static int DetailHeight(NotifItem n)
    {
        try
        {
            ...
            return Math.Clamp(64 + (int)sz.Height + 22, SummaryH + 26, 280);
        }
    ...
    public static void Draw(Graphics g, int w, int h, float a, NotifItem n, float detail, bool detailOn)
    {
        ...
        float tx = IconX + thumbW + 14, tw = w - tx - 22;
        float k = FontScale(n);
        using var appF = new Font("Segoe UI", 11f * k, GraphicsUnit.Pixel);
        using var titleF = new Font("Segoe UI Semibold", 16f * k, GraphicsUnit.Pixel);
        using var bodyF = new Font("Segoe UI", 13f * k, GraphicsUnit.Pixel);

        using (var b = new SolidBrush(Mul(Dim, a)))
        using (var f = LineFmt(n.App))
            g.DrawString(n.App, appF, b, new RectangleF(tx, 18, tw, 16), f);
        using (var b = new SolidBrush(Mul(White, a)))
        using (var f = LineFmt(n.Title))
            g.DrawString(n.Title, titleF, b, new RectangleF(tx, 36, tw, 22), f);

        if (detail < 0.999f && n.Body.Length > 0)
            using (var b = new SolidBrush(Mul(Dim, a * (1f - detail))))
            using (var f = LineFmt(n.Body))
                g.DrawString(n.Body, bodyF, b, new RectangleF(tx, 62, tw, 19), f);
        if (detail > 0.01f && n.Body.Length > 0)
            using (var b = new SolidBrush(Mul(Dim, a * detail)))
            using (var f = WrapFmt(n.Body))
                g.DrawString(n.Body, bodyF, b, new RectangleF(tx, 62, tw, h - 62 - 14), f);
