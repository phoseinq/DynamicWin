...
internal sealed class DownloadWidget : IWidget
{
    ...
    {
        var box = new RectangleF(ArtX, ArtY, ArtSize, ArtSize);
        using var path = Fx.Rounded(box, ArtSize * 0.24f);
        g.SmoothingMode = SmoothingMode.AntiAlias;
        if (icon != null)
        {
            int s = (int)Math.Ceiling(ArtSize);
        ...
        else
        {
            ...
            g.FillPath(gb, path);
            DrawGlyph(g, box, ((char)0xE896).ToString(), 46f, fade);
        }
    ...
    public void DrawCollapsed(Graphics g, int w, int h, float fade)
    {
        string? name = Downloads.Name;
        if (name == null) return;
        var accent = Ico() != null ? Accent() : Blue;
        float bx = 16, by = h / 2f - 3, bh = 6;
        if (Downloads.Installing)
        {
            float p = 0.5f + 0.5f * MathF.Sin(Environment.TickCount64 / 480f);
            float bw = w - bx - 16;
            Fill(g, bx, by, bw, bh, Track, bh / 2);
            float seg = bw * 0.38f, sx = bx + (bw - seg) * (0.5f + 0.5f * MathF.Sin(Environment.TickCount64 / 700f));
        ...
        float px = w - sz.Width - 14, py = (h - sz.Height) / 2f;
        g.DrawString(pct + "%", f, tb, px, py);
        float bw2 = px - bx - 12;
        Fill(g, bx, by, bw2, bh, Track, bh / 2);
        Fill(g, bx, by, bw2 * pct / 100f, bh, accent, bh / 2);
    }
