...
// OLE drag SOURCE: pick stashed file(s) up from the tray and drag them OUT to Explorer / another app.
// SHDoDragDrop shows the shell drag image (the file icons) under the cursor, not a bare rectangle.
// (The tray is also a drop TARGET — see FileDropTarget — for reveal-on-drag the other direction.)
internal static class FileDrag
{
    // true while WE are the drag source, so our own drop target ignores the pass-over (no self-reveal)
    ...
    // blocks in OLE's modal drag loop until the user drops or cancels — call on the STA UI thread.
    // returns true if the file(s) were actually dropped somewhere (so the tray can auto-remove them).
    ...
    private sealed class DropSource : Win32.IDropSource
    {
        ...
        public int QueryContinueDrag(int escapePressed, int keyState)
        {
            ...
            if ((keyState & MK_LBUTTON) == 0) return DRAGDROP_S_DROP; // button released → drop
            return S_OK;                                              // still held → keep dragging
        }
