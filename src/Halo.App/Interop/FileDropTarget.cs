...
// Real OLE drop target on the notch window. A file drag reveals the File Tray (WM_DROPFILES has no
// drag-enter, hence IDropTarget), and dropping stashes the paths. Only CF_HDROP (files) is accepted.
internal sealed class FileDropTarget : Win32.IDropTarget
{
    ...
    {
        if (FileDrag.Dragging) { effect = Win32.DROPEFFECT_NONE; return S_OK; } // our own drag-out passing over — ignore
