...
internal static class CodexDesktopCancel
{
    ...
    // PostMessage never reaches Electron's input pipeline (Chromium reads real input only),
    // so "posting" Esc = focus the ChatGPT window and send a genuine keystroke. The user just
    // clicked our stop button, so we hold foreground rights and the focus switch is expected.
    internal static bool Post(IntPtr handle, uint message, IntPtr wParam, IntPtr lParam)
    {
        if (message == WmKeyDown)
        {
            ...
            System.Threading.Thread.Sleep(80); // let the focus switch land before the key does
    ...
    private static bool SendKey(bool up)
    {
        var input = new INPUT
        {
            ...
            ki = new KEYBDINPUT { wVk = VkEscape, dwFlags = up ? 2u : 0u }, // KEYEVENTF_KEYUP
        };
    ...
    private struct INPUT
    {
        ...
        public long _pad; // pad to the union size (MOUSEINPUT is larger)
    }
