...
into widgets (media, mirrored notifications, file tray, downloads, Bluetooth battery, live Codex
...
dotnet run --project src\Halo.App -- --render-widget out.png Codex
...
| Hook | Renders |
|------|---------|
| `--render-widget <png> [media\|Codex\|codex]` | one widget's expanded panel |
...
**Agents (`Codex/` + `Codex/`)** are two near-mirrored modules, each with `Status.cs` (file
...
`~/.Codex/notch/status-{agentPid}.json` (plus `app.json` for the desktop surface; CLI wins when both
